...
function ActionLink({ target, children, quiet = false }: { readonly target: NavigationTarget; readonly children: ReactNode; readonly quiet?: boolean }) {
	return <a className={quiet ? "site-v1-home-action site-v1-home-action--quiet" : "site-v1-home-action"} href={resolveNavigationTarget("zh-cn", target)}>{children}</a>;
}
...
function HeroPicture() {
	...
	return (
		...
		>
			...
			<img src={heroAsset.master.src} alt="" width={heroAsset.master.width} height={heroAsset.master.height} fetchPriority="high" />
		</picture>
	);
}
	...
	return (
		<section className="site-v1-home-human-agent" data-home-section="human-agent" data-human-agent-bridge="home" data-fact-id={fact.id}>
			...
			<EvidenceLens
				...
				agentHref={`${getAgentPath("zh-cn", "home")}#${fact.id}`}
				presentation="signature"
			/>
			<div className="site-v1-home-actions">
				<a className="site-v1-home-action" href={getPublicPagePath("zh-cn", "human-agent")}>{copy.humanAgent.actions[0].label}</a>
				<a className="site-v1-home-action site-v1-home-action--quiet" href={`${getAgentPath("zh-cn", "home")}#${fact.id}`}>{copy.humanAgent.actions[1].label}</a>
			</div>
		</section>
	);
}
...
function CaseworkPreview() {
	...
	return (
		<section className="site-v1-casework-preview" data-home-section="casework" data-record-id={record.id} data-representative-record="casework-preview">
...
export function ChinaHomePage() {
	return (
		<ChineseSiteShell pageKey="home">
			<BuyerQuestionProvider record={record}>
				<div className="site-v1-home site-v1-home--zh">
					...
					<div id="product-preview" className="site-v1-home-product-anchor" tabIndex={-1}>
						<ProductRecordPreview copy={copy.productPreview} disclosure={copy.casework.disclosure} recordLabels={copy.siteV1.productRecord} stateLabels={copy.casework.stateLabels} />
					</div>
					<HomeEvidenceLens />
					<CaseworkPreview />
