...
import { firstContactInvalidField, type ContactFieldName, type ContactFieldRefs } from "./contact-fields";
...
function isHighIntentField(field: ContactFieldName | null): boolean {
	return field === "marketQuestion" || field === "marketOrLanguage" || field === "buyerOrCommercialContext";
}
...
export function useContactForm({ locale, requestType, initialResult, initialSubmissionId, fieldRefs, statusRef, validationCopy }: UseContactFormOptions) {
	...
	const pendingInvalidFocusRef = useRef<ContactFieldName | "form" | null>(null);

	useEffect(() => () => controllerRef.current?.abort(), []);
	useEffect(() => {
		if (state === "confirmed" || state === "unconfirmed") statusRef.current?.focus();
		if (state === "invalid" && pendingInvalidFocusRef.current) {
			const target = pendingInvalidFocusRef.current;
			pendingInvalidFocusRef.current = null;
			if (target === "form") statusRef.current?.focus();
			else fieldRefs.current[target]?.focus();
		}
	}, [errors, expanded, fieldRefs, state, statusRef]);
	...
	async function submit(event: React.FormEvent<HTMLFormElement>): Promise<void> {
		...
		const submittedBotField = new FormData(event.currentTarget).get("botField");
		const candidate = {
			...valuesRef.current,
			locale,
			requestType,
			botField: typeof submittedBotField === "string" ? submittedBotField : "",
		};
		const parsed = parseContactLead({ ...candidate, botField: "" });
		if (!parsed.success) {
			const nextErrors = contactFieldErrors({ ...candidate, botField: "" }, validationCopy);
			const first = firstContactInvalidField(nextErrors);
			pendingInvalidFocusRef.current = first ?? (nextErrors.form ? "form" : null);
			setErrors(nextErrors);
			if (isHighIntentField(first)) setExpanded(true);
			setState("invalid");
			return;
		}
		const outboundLead = candidate.botField ? { ...parsed.data, botField: candidate.botField } : parsed.data;
		...
		setErrors({});
		setSubmitting(true);
		const result = await submitContactRequest(outboundLead, identity.submissionId, { signal: controller.signal });
		...
		if (result.status === "invalid") {
			const first = firstContactInvalidField(result.fieldErrors);
			valuesRef.current = result.values;
			pendingInvalidFocusRef.current = first ?? (result.fieldErrors.form ? "form" : null);
			setValues(result.values);
			setErrors(result.fieldErrors);
			setExpanded(hasHighIntent(result.values) || isHighIntentField(first));
			setState("invalid");
			return;
		}
